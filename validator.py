    ...
    def validate_model(self):
        ...
        for i in range(1, self.models_count + 1):
            ...
            if self.validate_rf:
                ...
                rf_feature_data.append(joblib.load(self.rf_model_path + '_' + str(i) + '.dump').feature_importances_)
            if self.validate_lr:
                auc[1][i - 1], precision[1][i - 1], recall[1][i - 1] = self.validate_lr_model(X_val, y_val, i)
                lr_feature_data.append(joblib.load(self.lr_model_path + '_' + str(i) + '.dump').coef_[0])
        ...
        if self.validate_rf and rf_feature_data:
        ...
        if self.validate_lr and lr_feature_data:
